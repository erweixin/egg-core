...
module.exports = {
  ...
  loadConfig() {
    ...
    // 加载最外面目录的[config.default, config.prod]
    ...
    //             app config.{env}
    // this.getTypeFiles('config'): 返回值： [config.default, config.prod]
    // 循环各个加载单元中的config文件，所有属性都加载到target对象上。
    for (const filename of this.getTypeFiles('config')) {
      for (const unit of this.getLoadUnits()) {
        const isApp = unit.type === 'app';
        // {middleware: [ 'jsonWebToken' ], mongoose: {...}}
        // unit.path: 路径参数
        // filename：类似config.default或config.prod等
        // isApp ? undefined : appConfig： app时，最外层config作为加载app config的额外参数
        // unit.type
  ...
  // 合并config.default、config.${this.serverEnv}
  _preloadAppConfig() {
    const names = [
      'config.default',
  ...
  /**
     ...
     * @param extraInject
     * @param type
     * @return {*}  类似：{middleware: [ 'jsonWebToken' ], mongoose: {...}}
     * @private
     */
  _loadConfig(dirpath, filename, extraInject, type) {
    ...
    // 以this.appInfo为参数，运行filepath文件export出的函数。并将该函数运行的返回值赋值给config
