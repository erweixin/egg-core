...
class EggLoader {
   ...
   * @return {String} env enmu ['unittest', 'prod', 'local']
   * @see AppInfo#env
   * @private
   * @since 1.0.0
   */
   ...
   * 加载单个文件
   * 如果该文件exports的是一个函数，函数将默认以this.app为运行。并返回运行后的内容。
   * 如果该文件exports的是一个class直接返回该类。
   ...
   * 将文件目录内文件exports的属性加载到app[property]对象内
   ...
   * 将文件目录内文件exports的属性加载到app[property]、app.ctx.[property]对象内，并实现app.ctx.[property]的懒加载
  ...
  /**
     * 主要是使用自config配置文件
     * 默认加载config.default
     * 根据不同的环境，加载不同环境下配置文件config.prod等。
     *
     * @param filename
     * @returns {string[]} ep: [config.default, config.prod]
     */
  ...
  getTypeFiles(filename) {
    ...
    // ['unittest', 'prod', 'local']。为什么会有“default”?
